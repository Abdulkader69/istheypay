<div class="pay-sn-review-item">
    <div class="avatar"><img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt=""></div>
    <div class="pay-sn-review-context">
        <div class="pay-sn-review-metas">
            <div class="rat-title">
                <p class="rating">
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                </p>
                <h4>Reviewer Name</h4>
            </div>
            <div class="pay-about-sn">
                <p>
                    Offers
                    <span>5</span>
                </p>
                <p>
                    PAYOUT
                    <span>4</span>
                </p>
                <p>
                    TRACKING
                    <span>3</span>
                </p>
                <p>
                    SUPPORT
                    <span>5</span>
                </p>
            </div>
        </div>
        <div class="pay-sn-review-time">
            <p><?php the_time('M d y'); ?></p>
        </div>
        <div class="pay-sn-review-content">
            <p class="messages">Excellent company that always pay on time! Thank you for your TOP service, AdsMain! Please use this link for signup - https://bit.ly/3cPgiaB</p>
            <div class="paymrnt-proof">
                <img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt="">
            </div>
            <div class="like-dislike-btn">
                <p class="like"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
                <p class="dislike"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
            </div>
        </div>
    </div>
</div>


<div class="pay-sn-review-item">
    <div class="avatar"><img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt=""></div>
    <div class="pay-sn-review-context">
        <div class="pay-sn-review-metas">
            <div class="rat-title">
                <p class="rating">
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                </p>
                <h4>Reviewer Name</h4>
            </div>
            <div class="pay-about-sn">
                <p>
                    Offers
                    <span>5</span>
                </p>
                <p>
                    PAYOUT
                    <span>4</span>
                </p>
                <p>
                    TRACKING
                    <span>3</span>
                </p>
                <p>
                    SUPPORT
                    <span>5</span>
                </p>
            </div>
        </div>
        <div class="pay-sn-review-time">
            <p><?php the_time('M d y'); ?></p>
        </div>
        <div class="pay-sn-review-content">
            <p class="messages">Excellent company that always pay on time! Thank you for your TOP service, AdsMain! Please use this link for signup - https://bit.ly/3cPgiaB</p>
            <div class="paymrnt-proof">
                <img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt="">
            </div>
            <div class="like-dislike-btn">
                <p class="like"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
                <p class="dislike"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
            </div>
        </div>
    </div>
</div>


<div class="pay-sn-review-item">
    <div class="avatar"><img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt=""></div>
    <div class="pay-sn-review-context">
        <div class="pay-sn-review-metas">
            <div class="rat-title">
                <p class="rating">
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                </p>
                <h4>Reviewer Name</h4>
            </div>
            <div class="pay-about-sn">
                <p>
                    Offers
                    <span>5</span>
                </p>
                <p>
                    PAYOUT
                    <span>4</span>
                </p>
                <p>
                    TRACKING
                    <span>3</span>
                </p>
                <p>
                    SUPPORT
                    <span>5</span>
                </p>
            </div>
        </div>
        <div class="pay-sn-review-time">
            <p><?php the_time('M d y'); ?></p>
        </div>
        <div class="pay-sn-review-content">
            <p class="messages">Excellent company that always pay on time! Thank you for your TOP service, AdsMain! Please use this link for signup - https://bit.ly/3cPgiaB</p>
            <div class="paymrnt-proof">
                <img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt="">
            </div>
            <div class="like-dislike-btn">
                <p class="like"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
                <p class="dislike"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
            </div>
        </div>
    </div>
</div>


<div class="pay-sn-review-item">
    <div class="avatar"><img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt=""></div>
    <div class="pay-sn-review-context">
        <div class="pay-sn-review-metas">
            <div class="rat-title">
                <p class="rating">
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                </p>
                <h4>Reviewer Name</h4>
            </div>
            <div class="pay-about-sn">
                <p>
                    Offers
                    <span>5</span>
                </p>
                <p>
                    PAYOUT
                    <span>4</span>
                </p>
                <p>
                    TRACKING
                    <span>3</span>
                </p>
                <p>
                    SUPPORT
                    <span>5</span>
                </p>
            </div>
        </div>
        <div class="pay-sn-review-time">
            <p><?php the_time('M d y'); ?></p>
        </div>
        <div class="pay-sn-review-content">
            <p class="messages">Excellent company that always pay on time! Thank you for your TOP service, AdsMain! Please use this link for signup - https://bit.ly/3cPgiaB</p>
            <div class="paymrnt-proof">
                <img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt="">
            </div>
            <div class="like-dislike-btn">
                <p class="like"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
                <p class="dislike"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
            </div>
        </div>
    </div>
</div>


<div class="pay-sn-review-item">
    <div class="avatar"><img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt=""></div>
    <div class="pay-sn-review-context">
        <div class="pay-sn-review-metas">
            <div class="rat-title">
                <p class="rating">
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                    <span><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""></span>
                </p>
                <h4>Reviewer Name</h4>
            </div>
            <div class="pay-about-sn">
                <p>
                    Offers
                    <span>5</span>
                </p>
                <p>
                    PAYOUT
                    <span>4</span>
                </p>
                <p>
                    TRACKING
                    <span>3</span>
                </p>
                <p>
                    SUPPORT
                    <span>5</span>
                </p>
            </div>
        </div>
        <div class="pay-sn-review-time">
            <p><?php the_time('M d y'); ?></p>
        </div>
        <div class="pay-sn-review-content">
            <p class="messages">Excellent company that always pay on time! Thank you for your TOP service, AdsMain! Please use this link for signup - https://bit.ly/3cPgiaB</p>
            <div class="paymrnt-proof">
                <img src="<?php echo site_url(); ?>/wp-content/uploads/2021/03/avatar3.png" alt="">
            </div>
            <div class="like-dislike-btn">
                <p class="like"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
                <p class="dislike"><img width="15px" height="15px" src="<?php bloginfo('stylesheet_directory'); ?>/assets/images/star.svg" alt=""> (0)</p>
            </div>
        </div>
    </div>
</div>